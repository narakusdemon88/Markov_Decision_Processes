...
def value_iteration(gamma_range: list, probabilities, rewards):
    ...
    for gamma in gamma_range:
        ...
        value_iteration = mdp.ValueIteration(
        ...
        score = float(sum(value_iteration.V) / len(value_iteration.V))
        policy = value_iteration.policy

        one_value["iteration"] = value_iteration.iter
...
def calculate_rewards(probabilities, rewards):
    main_policy = mdp.ValueIteration(
        transitions=probabilities,
        reward=rewards,
        gamma=.999,
        epsilon=.0001,
...
def plot_convergence(rolling):
    ...
    plt.title(f"Convergence (VI) Frozen Lake")
    plt.tight_layout()
    plt.grid(visible=True)
    plt.show()
    plt.clf()
